...
% with genHyBRs (iterative + regularization)
%
...
for j = 1:nRO
    for i = 1:nNO
        outfile_fluxes{i,j} = strcat(outpath,'Fluxes_genHyBR_',num2str(NoiseOptions(i)),'_',RegOptions(j),'_',date,'_',num2str(iter),'.csv');
        outfile_error{i,j} = strcat(outpath,'Error_genHyBR_',num2str(NoiseOptions(i)),'_',RegOptions(j),'_',date,'_',num2str(iter),'.csv');
        outfile_res{i,j} = strcat(outpath,'Res_genHyBR_',num2str(NoiseOptions(i)),'_',RegOptions(j),'_',date,'_',num2str(iter),'.csv');
        outfile_output{i,j} = strcat(outpath,'Output_genHyBR_',num2str(NoiseOptions(i)),'_',RegOptions(j),'_',date,'_',num2str(iter),'.mat');
    end
end
...
parfor k = 1 : (nNO*nRO)
    ...
    switch char(parRegOptions(k))
        case 'LSQR'
            ...
            input = HyBR_lsmrset('InSolv', 'none',...
                         'Reorth','on','Iter', maxit,'x_true',s_true,'nLevel',theta(1));
        otherwise
            disp(strcat('Run_', char(parRegOptions(k)), '_for_', num2str(parNoiseOptions(k)), '%_noise_level' ));
    ...
    % selection methods
    tic;
    [recons, output] = genHyBR(H, Z(:), Qkron, R, input);
    toc;
